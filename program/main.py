...
if __name__ == "__main__":
    ...
    pipeline = Pipeline(
        ...
        gamma_distrib_bins=config["gamma_distribution_bins"],
        del_aox_distribution_binwidth=config["del_aox_distribution_binwidth"],
        best_fit_min_dof=config["best_fit_min_dof"],
    )
    ...
    if steps.get("post_analysis_plots", False):
        pipeline.post_analysis_plots()
        
    if steps.get("count_plots", False):
        pipeline.count_plots()

    if steps.get("literature_check", False):
        pipeline.literature_check()
        pipeline.literature_consistency_check()
