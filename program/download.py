...
def _Single_Downloader(self, obsid):
    ...
    try:
        ...
        self.logger.info(f"{obsid}: download failed, removing incomplete directory and skipping")
        shutil.rmtree(obsid_dir, ignore_errors=True)
        return

    archives = list(obsid_dir.glob("*.tar.gz")) + list(obsid_dir.glob("*.tar"))
    if not archives:
        self.logger.info(f"{obsid}: no archive found after download, removing incomplete directory and skipping")
        shutil.rmtree(obsid_dir, ignore_errors=True)
        return
    archive = archives[0]
...
def Download_Catalogue(self):
    ...
    if xmm_path.exists():
        self.logger.info("5XMM-DR15 already downloaded, skipping")
    else:
        self.logger.info("Downloading 5XMM-DR15 catalogue...")
        url = "http://xmmssc.irap.omp.eu/Catalogue/5XMM-DR15/5XMM_DR15cat_source_v1.0.fits.gz"
        response = requests.get(url, stream=True)
        with open(xmm_path, "wb") as f:
            for chunk in response.iter_content(chunk_size=8192):
                f.write(chunk)
        self.logger.info("5XMM-DR15 downloaded")
    if sdss_path.exists():
