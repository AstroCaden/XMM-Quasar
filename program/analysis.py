...
def _match_aox_row(self, target_name, ra, dec, aox_table):
    ...
    coords = SkyCoord(ra=aox_table["RA"] * u.deg, dec=aox_table["DEC"] * u.deg)
...
def xray_weakness_comparison(self):
    ...
    for name, data in targets.items():
        ...
        gamma_pn = gamma_pn_lo = gamma_pn_hi = dof_pn = None
        gamma_combined = gamma_combined_lo = gamma_combined_hi = dof_combined = None
        gamma_mos = gamma_mos_lo = gamma_mos_hi = dof_mos = None
        hr_median = hr_lo = hr_hi = None
        fvar_val = fvar_err = None
        ...
        if not spec_path.exists():
            ...
            best_rcstat_pn = best_rcstat_combined = best_rcstat_mos = float("inf")
            best_rcstat_pn_any = best_rcstat_combined_any = best_rcstat_mos_any = float("inf")
            gamma_pn_any = gamma_pn_any_lo = gamma_pn_any_hi = dof_pn_any = None
            gamma_combined_any = gamma_combined_any_lo = gamma_combined_any_hi = dof_combined_any = None
            gamma_mos_any = gamma_mos_any_lo = gamma_mos_any_hi = dof_mos_any = None
            with open(spec_path, newline="") as f:
                for r in csv.DictReader(f):
                    dof_val = float(r["dof"])
                    if dof_val <= 0:
                        continue
                    rcstat = float(r["cstat"]) / dof_val
 
                    if r["fit_type"] == "pn" and rcstat < best_rcstat_pn_any:
                        best_rcstat_pn_any = rcstat
                        gamma_pn_any = float(r["gamma"])
                        gamma_pn_any_lo = float(r["gamma_lo"])
                        gamma_pn_any_hi = float(r["gamma_hi"])
                        dof_pn_any = dof_val
                    if r["fit_type"] == "pn" and dof_val >= self.best_fit_min_dof and rcstat < best_rcstat_pn:
                        best_rcstat_pn = rcstat
                        gamma_pn = float(r["gamma"])
                        gamma_pn_lo = float(r["gamma_lo"])
                        gamma_pn_hi = float(r["gamma_hi"])
                        dof_pn = dof_val
 
                    if r["fit_type"] == "combined" and rcstat < best_rcstat_combined_any:
                        best_rcstat_combined_any = rcstat
                        gamma_combined_any = float(r["gamma"])
                        gamma_combined_any_lo = float(r["gamma_lo"])
                        gamma_combined_any_hi = float(r["gamma_hi"])
                        dof_combined_any = dof_val
                    if r["fit_type"] == "combined" and dof_val >= self.best_fit_min_dof and rcstat < best_rcstat_combined:
                        best_rcstat_combined = rcstat
                        gamma_combined = float(r["gamma"])
                        gamma_combined_lo = float(r["gamma_lo"])
                        gamma_combined_hi = float(r["gamma_hi"])
                        dof_combined = dof_val
 
                    if r["fit_type"] == "mos" and rcstat < best_rcstat_mos_any:
                        best_rcstat_mos_any = rcstat
                        gamma_mos_any = float(r["gamma"])
                        gamma_mos_any_lo = float(r["gamma_lo"])
                        gamma_mos_any_hi = float(r["gamma_hi"])
                        dof_mos_any = dof_val
                    if r["fit_type"] == "mos" and dof_val >= self.best_fit_min_dof and rcstat < best_rcstat_mos:
                        best_rcstat_mos = rcstat
                        gamma_mos = float(r["gamma"])
                        gamma_mos_lo = float(r["gamma_lo"])
                        gamma_mos_hi = float(r["gamma_hi"])
                        dof_mos = dof_val
 
            if gamma_pn is None and gamma_pn_any is not None:
                self.logger.info(f"{name} pn: no fit with dof>={self.best_fit_min_dof}, falling back to best available (dof={dof_pn_any:.0f})")
                gamma_pn, gamma_pn_lo, gamma_pn_hi, dof_pn = gamma_pn_any, gamma_pn_any_lo, gamma_pn_any_hi, dof_pn_any
            if gamma_combined is None and gamma_combined_any is not None:
                self.logger.info(f"{name} combined: no fit with dof>={self.best_fit_min_dof}, falling back to best available (dof={dof_combined_any:.0f})")
                gamma_combined, gamma_combined_lo, gamma_combined_hi, dof_combined = gamma_combined_any, gamma_combined_any_lo, gamma_combined_any_hi, dof_combined_any
            if gamma_mos is None and gamma_mos_any is not None:
                self.logger.info(f"{name} mos: no fit with dof>={self.best_fit_min_dof}, falling back to best available (dof={dof_mos_any:.0f})")
                gamma_mos, gamma_mos_lo, gamma_mos_hi, dof_mos = gamma_mos_any, gamma_mos_any_lo, gamma_mos_any_hi, dof_mos_any
        ...
        if not hr_path.exists():
            ...
            with open(hr_path, newline="") as f:
                for r in csv.DictReader(f):
                    total = float(r["src_soft"]) + float(r["src_hard"])
                    if total > best_total:
                        best_total = total
        ...
        results.append({
            ...
            "gamma_pn": gamma_pn,
            "gamma_pn_lo": gamma_pn_lo,
            "gamma_pn_hi": gamma_pn_hi,
            "dof_pn": dof_pn,
            "gamma_combined": gamma_combined,
            "gamma_combined_lo": gamma_combined_lo,
            "gamma_combined_hi": gamma_combined_hi,
            "dof_combined": dof_combined,
            "gamma_mos": gamma_mos,
            "gamma_mos_lo": gamma_mos_lo,
            "gamma_mos_hi": gamma_mos_hi,
            "dof_mos": dof_mos,
            "hr_median": hr_median,
            "hr_lo": hr_lo,
            "hr_hi": hr_hi,
        ...
        self.logger.info(f"{name}: {classification} (Δαox={delta_aox_str}), Γ_pn={gamma_pn}, Γ_combined={gamma_combined}, Γ_mos={gamma_mos}, HR={hr_median}, Fvar={fvar_val}")
