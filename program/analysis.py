...
def _hardness_ratio(self):
    ...
    for obsid in self.ObsID_current:
        ...
        soft = (channel >= self.hr_soft_min) & (channel < self.hr_soft_max)
        hard = (channel >= self.hr_hard_min) & (channel < self.hr_hard_max)
    ...
    for obsid in self.ObsID_current:
        ...
        for fit_type, spec_list in [("combined", combined_list), ("pn", pn_list), ("mos", mos_list)]:
            ...
            m.zpowerlw.PhoIndex.values = (
                ...
                self.specfit_gamma_min - 0.5, self.specfit_gamma_min,
                self.specfit_gamma_max, self.specfit_gamma_max + 0.5,)
...
def xray_weakness_comparison(self):
    ...
    for name, data in targets.items():
        ...
        if row is None:
            self.logger.info(f"{name}: no match in aox_catalogue, skipping")
            continue

        delta_aox = row["DELTA_AOX"]
        classification = "weak" if delta_aox < self.xray_weak_threshold else "normal"

        star_folder = self.base_dir / "Datasets" / name.replace(" ", "_").replace("+", "_")

        gamma = gamma_lo = gamma_hi = None
        hr_median = hr_lo = hr_hi = None
        fvar_val = fvar_err = None

        spec_path = star_folder / "spectral_fits.csv"
        if not spec_path.exists():
            self.logger.info(f"{name}: spectral_fits.csv not found, please run spectral fitting for this target first")
        else:
            with open(spec_path, newline="") as f:
                for r in csv.DictReader(f):
                    if r["fit_type"] == "pn":
                        gamma = float(r["gamma"])
                        gamma_lo = float(r["gamma_lo"])
                        gamma_hi = float(r["gamma_hi"])
                        break

        hr_path = star_folder / "hardness_ratios.csv"
        if not hr_path.exists():
            self.logger.info(f"{name}: hardness_ratios.csv not found, please run hardness ratio calculation for this target first")
        else:
        ...
        fvar_path = star_folder / "fvar.csv"
        if not fvar_path.exists():
            self.logger.info(f"{name}: fvar.csv not found, please run Fvar calculation for this target first")
        else:
        ...
        self.logger.info(f"{name}: {classification} (Δαox={delta_aox:.2f}), Γ={gamma}, HR={hr_median}, Fvar={fvar_val}")

    if results:
        out_path = self.base_dir / "Datasets" /"xray_weakness_comparison.csv"
        with open(out_path, "w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=results[0].keys())
            writer.writeheader()
            writer.writerows(results)
