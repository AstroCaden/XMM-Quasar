...
from program.plotting import post_analysis_plots
from program.plotting import count_plots
from program.literature import literature_check
from program.literature import literature_consistency_check
...
class Pipeline:
    ...
    def __init__(
        ...
        self.xmm_band3_width=xmm_band3_width
        self.xmm_band4_width=xmm_band4_width
        self.gamma_distrib_bins = gamma_distrib_bins
        self.del_aox_distribution_binwidth=del_aox_distribution_binwidth
        self.best_fit_min_dof=best_fit_min_dof
    ...
    def _Reprocessing(self):
        ...
        if isinstance(reprocess, str):
            if reprocess not in ("true", "false", "careful"):
                raise ValueError(f"reprocess must be true, false, or 'careful', got {reprocess!r}")
            reprocess = {"true": True, "false": False, "careful": "careful"}[reprocess]
        elif reprocess not in (True, False):
            raise ValueError(f"reprocess must be true, false, or 'careful', got {self.reprocess!r}")

        if not reprocess:
            return
        if reprocess is not True:
            confirm = input(f"reprocess is set to {reprocess!r}: this will delete existing processed files and cannot be undone. Continue? [y/N] ")
            if confirm.strip().lower() != "y":
                self.logger.info("Reprocessing cancelled")
                return
...
Pipeline.post_analysis_plots = post_analysis_plots
Pipeline.count_plots = count_plots
Pipeline.literature_check = literature_check
Pipeline.literature_consistency_check = literature_consistency_check
