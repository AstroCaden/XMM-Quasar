    ...
    for obsid in self.ObsID_current:
        ...
        if raw_counts < min_counts:
            ...
            if self.stop_at_low_counts:
                self.logger.info(f"{obsid}: stop_at_low_counts is true, terminating")
                sys.exit(0)
...
def _background_region(self, fits_dir, src_x, src_y):
    cat_path = self.base_dir / "Catalogues" / "4XMM_slim_DR14.fits.gz"

    bkg_region = f"(X,Y) IN annulus({src_x},{src_y},{self.bkg_radius_in},{self.bkg_radius_out})"

    if not cat_path.exists():
        self.logger.info("4XMM catalogue not found, using plain background annulus")
        return bkg_region

    with fits.open(fits_dir / "pn_clean.ds") as hdul:
        hdr = hdul["EVENTS"].header
        ra_ref  = hdr["REFXCRVL"]
        dec_ref = hdr["REFYCRVL"]
        x_ref   = hdr["REFXCRPX"]
    ...
    cat_coords = SkyCoord(ra=cat["SC_RA"] * u.deg, dec=cat["SC_DEC"] * u.deg)
    ...
    for row in contaminants:
        ...
        bkg_region += f" && !((X,Y) IN circle({cont_x},{cont_y},{exclude_radius}))"
