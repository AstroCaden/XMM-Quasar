...
def _calculate_del_aox(self):
    ...
    t = t[t["SC_SUM_FLAG"] < self.quality_sum_flag_max]
...
def lxluv_plot(self, photon_index):
    ...
    ax.plot(x_fit, y_fit + self.xray_weak_threshold, color="blue", linewidth=0.8, linestyle="--")
    ax.plot(x_fit, y_fit + self.xray_strong_threshold, color="red", linewidth=0.8, linestyle="--")
...
def _civ_correlation(self, photon_index, civ_bins):
    ...
    plt.suptitle(f"4XMM-DR14 × SDSS DR16  |  z = {self.z_min}–{self.z_max}, Γ = {photon_index}  |  {self.field_type}", fontsize=12)
    plt.tight_layout()
...
def _delta_aox_distribution(self, photon_index):
    ...
    ax.axvline(threshold, color="gray", linestyle=":", linewidth=1)
    ax.axvline(self.xray_weak_threshold, color="red", linestyle="--", linewidth=0.8)
    ax.axvline(self.xray_strong_threshold, color="blue", linestyle="--", linewidth=0.8)
